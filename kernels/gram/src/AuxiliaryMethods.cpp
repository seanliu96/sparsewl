...
GraphDatabase read_graph_txt_file(string data_set_dir, string data_set_name) {
    string line;

    vector<uint> graph_indicator;
    ifstream myfile(data_set_dir + "/" + data_set_name + "/" + data_set_name + "_graph_indicator.txt");
    ...
    string label;
    Labels node_labels;
    ifstream labels(data_set_dir + "/" + data_set_name + "/" + data_set_name + "_node_labels.txt");
    if (labels.is_open()) {
    ...
    ifstream attributes(data_set_dir + "/" + data_set_name + "/" + data_set_name + "_node_attributes.txt");
    ...
    Labels edge_labels;
    ifstream elabels(data_set_dir + "/" + data_set_name + "/" + data_set_name + "_edge_labels.txt");
    if (elabels.is_open()) {
    ...
    ifstream eattr(data_set_dir + "/" + data_set_name + "/" + data_set_name + "_edge_attributes.txt");
    if (eattr.is_open()) {
    ...
    uint c = 0;
    vector<int> edges;
    ifstream edge_file(data_set_dir + "/" + data_set_name + "/" + data_set_name + "_A.txt");
...
vector<int> read_classes(string data_set_dir, string data_set_name) {
    string line;

    string path = "./datasets/";
    vector<int> classes;

    ifstream myfile(data_set_dir + "/" + data_set_name + "/" + data_set_name + "_graph_labels.txt");
    if (myfile.is_open()) {
        while (getline(myfile, line)) {
...
vector<vector<float>> read_multi_targets(string data_set_dir, string data_set_name) {
    string line;

    string path = ".";
    vector<vector<float>> targets;

    ifstream myfile(data_set_dir + "/" + "/datasets/" + data_set_name + "/" + data_set_name + "_graph_attributes.txt");
    if (myfile.is_open()) {
        while (getline(myfile, line)) {
...
void write_sparse_gram_matrix(const GramMatrix &gram_matrix, string file_name) { saveMarket(gram_matrix, file_name); }

void write_libsvm(const GramMatrix &gram_matrix, const vector<int> classes, string filename) {
    MatrixXd dense_gram_matrix(gram_matrix);
